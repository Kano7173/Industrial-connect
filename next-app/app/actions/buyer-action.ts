...
export async function buyerAction(formData:FormData){
 const user=await requireCurrentUser(); if(user.role!=='BUYER')throw new Error('Only buyers can make an inspection decision.');
 ...
 await prisma.$transaction(async tx=>{const now=new Date();if(input.decision==='APPROVE'){
 ...
 revalidatePath(`/dashboard/buyer/${order.id}`);revalidatePath(`/dashboard/supplier/${order.id}`);return{ok:true};
}
