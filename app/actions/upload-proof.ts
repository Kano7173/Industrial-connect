...
const schema = z.object({ orderId: z.string().min(1), stepName: z.enum(['RAW_MATERIAL_BILL', 'FACTORY_VIDEO', 'LR_COPY']), fileUrl: z.string().url().max(2048) });
const nextStatus = { RAW_MATERIAL_BILL: 'MATERIAL_UPLOADED', FACTORY_VIDEO: 'PRODUCTION_VIDEO_ADDED', LR_COPY: 'DISPATCHED' } as const;
const rank: Record<string, number> = { PENDING_ESCROW: 0, ESCROW_LOCKED: 1, MATERIAL_UPLOADED: 2, PRODUCTION_VIDEO_ADDED: 3, DISPATCHED: 4, COMPLETED: 5, DISPUTED: 5, REFUNDED: 5 };

export async function uploadProof(formData: FormData): Promise<void> {
  const user = await requireCurrentUser();
  if (user.role !== 'SUPPLIER') throw new Error('Only suppliers can upload production proofs.');
  const input = schema.parse({ orderId: formData.get('orderId'), stepName: formData.get('stepName'), fileUrl: formData.get('fileUrl') });
  const order = await prisma.order.findUnique({ where: { id: input.orderId } });
  if (!order || order.supplierId !== user.id) throw new Error('Order not found or access denied.');
  if (['COMPLETED', 'DISPUTED', 'REFUNDED'].includes(order.status)) throw new Error('This order is locked.');
